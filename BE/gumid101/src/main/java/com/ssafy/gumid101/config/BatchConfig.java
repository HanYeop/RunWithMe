...
public class BatchConfig {
	...
	public Step pointCalculatingStep() {
		...
		return stepBuilderFactory.get("pointCalculatingStep").<CrewEntity,CrewEntity>chunk(CHUNK_SIZE).reader(endCrewReader(null))
				.processor(endCrewPointCalculateProcessor()).writer(calculatedPintWriter()).build();
	...
	@Bean
	@StepScope
	public JpaPagingItemReader<CrewEntity> endCrewReader(@Value("#{jobParameters[requestDate]}") final String requestDate) {
		...
		return new JpaPagingItemReaderBuilder<CrewEntity>()
				.pageSize(CHUNK_SIZE)
				.parameterValues(parameterValues)
				.queryString("SELECT c FROM CrewEntity c where c.crewCheckYn = :crewCheckYn AND c.crewDateEnd < :nowTime ORDER BY c.crewSeq")
				.entityManagerFactory(entityManagerFactory)
				.name("crewPointCalculationReader")
				.build();
	...
	@Bean
	@StepScope //스텝이 보는 영역
	public ItemWriter<CrewEntity> calculatedPintWriter() {
		log.debug("포인트 정산 JOB-Writer 과정을 시작합니다.");
		 return new JpaItemWriterBuilder<CrewEntity>()
	                .entityManagerFactory(entityManagerFactory)
	                .build();
	}
